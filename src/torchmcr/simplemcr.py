...
class SimpleMCRModel(MCR):
    ...
    def freeze_weights(self, indices=None):
        """
        Freeze gradients for the specified weights or for the entire weights matrix if no indices are provided.

        Parameters:
            indices (list, optional): List of row indices in the weights matrix to freeze. If None, freezes all weights.
        """
        if indices is None:
            ...
            with torch.no_grad():
                for idx in indices:
                    self.weights.weight_matrix[idx].requires_grad = False
    def unfreeze_weights(self, indices=None):
        """
        Freeze gradients for the specified weights or for the entire weights matrix if no indices are provided.
        ...
        if indices is None:
            ...
            with torch.no_grad():
                for idx in indices:
                    self.weights.weight_matrix[idx].requires_grad = True


    def freeze_spectra(self, indices=None):
        """
        Freeze gradients for the specified spectra or for the entire spectra matrix if no indices are provided.

        Parameters:
            indices (list, optional): List of row indices in the spectra matrix to freeze. If None, freezes all spectra.
        """
        if indices is None:
            ...
            with torch.no_grad():
                for idx in indices:
                    self.spectra.spectra_matrix[idx].requires_grad = False

    def unfreeze_spectra(self, indices=None):
        """
        Freeze gradients for the specified spectra or for the entire spectra matrix if no indices are provided.
        ...
        if indices is None:
            ...
            with torch.no_grad():
                for idx in indices:
                    self.spectra.spectra_matrix[idx].requires_grad = True